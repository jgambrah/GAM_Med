...
// ─── Category Types ───────────────────────────────────────────────────────────
type MenuCategory =
  | 'general'       // Dashboard + Messages — always visible
  | 'platform'      // Super-admin only
  | 'my_practice'   // Doctor-specific quick links
  | 'clinical'      // All patient-facing workflows
  | 'diagnostics'   // Lab, Radiology
  | 'theatre'       // OT, Surgery
  | 'pharmacy'      // Pharmacy + Controlled substances
  | 'finance'       // Finance hub, billing, payroll
  | 'admin'         // Staff, HR, approvals, compliance
  | 'logistics'     // Inventory, Equipment, Suppliers, Space
...
export function MainNavClient() {
  ...
  const accessibleItems = menuItems.filter(item => {
    ...
    // Explicit string cast to prevent TypeScript narrowing issues
          ...
          <Collapsible key={cat.id} defaultOpen={shouldDefaultOpen} className="group/collapsible mb-2">
            <SidebarMenuItem>
              <CollapsibleTrigger asChild>
                <SidebarMenuButton
                  tooltip={cat.label}
                  className="font-black text-xs text-slate-500 hover:text-primary"
                >
