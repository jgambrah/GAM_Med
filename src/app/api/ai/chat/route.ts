...
export function ClinicalAssistant() {
  ...
  const encountersQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'hospitals', userProfile.hospitalId, 'patients', patientId, 'encounters'), orderBy('createdAt', 'desc'), limit(5));
  ...
  const labResultsQuery = useMemoFirebase(() => {
    if (!firestore || !userProfile?.hospitalId || !patientId || Array.isArray(patientId)) return null;
    return query(collectionGroup(firestore, 'lab_orders'), where('patientId', '==', patientId), where('status', '==', 'COMPLETED'), limit(3));
  }, [firestore, userProfile?.hospitalId, patientId]);
