...
import { 
  ...
  Printer, MessageSquare, Check, Radio, Share2, ArrowLeft
} from 'lucide-react';
...
export default function ClinicalAssignmentQueue() {
  ...
  const officerName = userProfile?.fullName || (isNurse ? 'Ama Takyi' : 'Jessica Bansah');
  const officerInitials = officerName.split(' ').map((n: string) => n[0]).slice(0, 2).join('').toUpperCase() || (isNurse ? 'AT' : 'JB');
  const officerRoleTitle = isNurse 
    ? 'NURSE (OPD / TRIAGE DISPATCH)' 
    : isDoctor 
    ? 'CLINICAL DISPATCH' 
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-3">
              <div className={cn(
                "p-2.5 border rounded-2xl",
                isNurse ? "bg-rose-500/20 border-rose-500/30 text-rose-400" : "bg-indigo-500/20 border-indigo-500/30 text-indigo-400"
              )}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className={cn(
                    ...
                    isNurse ? "bg-rose-500/20 text-rose-300 border-rose-500/30" : "bg-indigo-500/20 text-indigo-300 border-indigo-500/30"
                  )}>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-1.5">
              ...
              {isNurse && (
                <Link
                  href="/nurse/opd"
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-[11px] font-black uppercase tracking-wider transition"
                >
                  <ArrowLeft className="w-3 h-3" />
                  <span>Return to OPD Desk</span>
                </Link>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-slate-900 border border-slate-800 rounded-2xl px-4 py-2.5 self-start md:self-auto shadow-inner">
            <div className={cn(
              "w-9 h-9 rounded-full border flex items-center justify-center font-black text-xs",
              isNurse ? "bg-rose-950 border-rose-700/60 text-rose-400" : "bg-indigo-950 border-indigo-700/60 text-indigo-400"
            )}>
              {officerInitials}
            </div>
            <div>
              ...
              <div className={cn(
                ...
                isNurse ? "text-rose-400" : "text-indigo-400"
              )}>
                {officerRoleTitle}
              </div>
            </div>
      ...
      {routedSuccessData && (
        <Dialog open={!!routedSuccessData} onOpenChange={() => setRoutedSuccessData(null)}>
          <DialogContent className="bg-slate-950 border border-slate-800 text-white rounded-3xl p-6 md:p-8 max-w-lg shadow-2xl space-y-5">
            ...
            {/* Done & Return Action Buttons */}
            <div className="pt-2 flex flex-col sm:flex-row gap-2">
              <button
                type="button"
                onClick={() => setRoutedSuccessData(null)}
                className="flex-1 py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-black text-xs rounded-xl shadow-lg transition flex items-center justify-center gap-2 uppercase tracking-wider cursor-pointer"
              >
              ...
              {isNurse && (
                <button
                  type="button"
                  onClick={() => {
                    setRoutedSuccessData(null);
                    router.push('/nurse/opd');
                  }}
                  className="py-3.5 px-4 bg-rose-600 hover:bg-rose-500 text-white font-black text-xs rounded-xl shadow-lg transition flex items-center justify-center gap-2 uppercase tracking-wider cursor-pointer"
                >
