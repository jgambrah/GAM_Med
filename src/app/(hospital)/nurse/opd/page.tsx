...
// Active Consulting Rooms Telemetry
const OPD_CONSULTING_ROOMS = [
  { room: 'Consulting Room 1', doctor: 'Dr. Ama Adu', specialty: 'GP / Family Med', load: 2, status: 'IN SESSION' },
  { room: 'Consulting Room 2', doctor: 'Dr. Richard Yeboah', specialty: 'Medical Officer', load: 4, status: 'BUSY' },
  { room: 'Consulting Room 4', doctor: 'Dr. Tracy Gambrah', specialty: 'Lead Physician', load: 1, status: 'AVAILABLE' },
  { room: 'Consulting Room 5', doctor: 'Dr. James Obrempong', specialty: 'Surgical OPD', load: 0, status: 'OPEN' },
];
...
export default function OutpatientDeskPage() {
  ...
  const [stageFilter, setStageFilter] = useState<'ALL' | 'WAITING_DOC' | 'IN_CONSULT' | 'VITALS_DONE'>('ALL');
  ...
  const filteredPatients = useMemo(() => {
    return DEMO_OPD_PATIENTS.filter(p => {
      ...
      if (stageFilter === 'VITALS_DONE' && p.stage !== 'Vitals Logged') return false;
  ...
  const totalPatients = DEMO_OPD_PATIENTS.length;
  const inConsultCount = DEMO_OPD_PATIENTS.filter(p => p.stage === 'In Consultation').length;
  const waitingDocCount = DEMO_OPD_PATIENTS.filter(p => p.stage === 'Awaiting Doctor' || p.stage === 'Vitals Logged').length;
  const urgentCount = DEMO_OPD_PATIENTS.filter(p => p.acuity === 'URGENT').length;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="mt-1 text-xs md:text-sm text-slate-400 font-medium max-w-2xl">
              Real-time outpatient consultation monitoring, consulting room telemetry, and post-triage patient flow tracking.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-white">{totalPatients}</div>
              <span className="text-[10px] font-bold text-slate-400 mt-1 block">Active OPD Visits</span>
            </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-amber-400">{waitingDocCount}</div>
              <span className="text-[10px] font-bold text-slate-400 mt-1 block">Vitals Cleared</span>
            </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-rose-400">{urgentCount}</div>
              <span className="text-[10px] font-bold text-slate-400 mt-1 block">Fever or Elevated BP</span>
            </div>
      ...
      {/* 2. Consulting Rooms Live Telemetry */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">Live queue load across outpatient consulting suites.</p>
          </div>
          <Link 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {OPD_CONSULTING_ROOMS.map((crm, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-[10px] font-mono">
                ...
                <span className="font-bold text-indigo-600 dark:text-indigo-400">{crm.load} Patients</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* 3. OPD Patient Roster & Stage Filter */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            All OPD ({DEMO_OPD_PATIENTS.length})
          </button>
          <button
            type="button"
          ...
          >
            Awaiting Doctor ({DEMO_OPD_PATIENTS.filter(p => p.stage === 'Awaiting Doctor').length})
          </button>
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={() => setStageFilter('VITALS_DONE')}
            className={cn(
              "px-3 py-1.5 rounded-xl text-xs font-bold transition",
              stageFilter === 'VITALS_DONE' 
                ? "bg-blue-600 text-white shadow-sm"
                : "bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-900/60"
            )}
          >
            Vitals Logged ({DEMO_OPD_PATIENTS.filter(p => p.stage === 'Vitals Logged').length})
          </button>
        </div>
              ...
              {filteredPatients.map((patient) => (
                <tr key={patient.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition">
                  <td className="p-3.5">
                    ...
                    <div className="text-[10px] text-slate-500 font-mono">{patient.ehr} • {patient.age}y / {patient.gender}</div>
                  </td>
                  <td className="p-3.5">
                    <div className="font-bold text-slate-800 dark:text-slate-200">BP: {patient.vitals.bp} • {patient.vitals.temp}</div>
                    <div className="text-[10px] text-slate-500">Pulse: {patient.vitals.pulse} • SpO2: {patient.vitals.spo2}</div>
                  </td>
                  ...
                  <td className="p-3.5 text-right">
                    <Link
                      href={`/nurse/triage`}
                      className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-lg transition inline-flex items-center gap-1"
                    >
                      <span>Vitals</span>
                      <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
