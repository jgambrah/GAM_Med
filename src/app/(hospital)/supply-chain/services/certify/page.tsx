...
export default function JobCompletionCertification() {
  ...
  const [certifyingPO, setCertifyingPO] = useState<any | null>(null);
  ...
  const certifyService = async (po: any) => {
    ...
    setLoading(true);
    setCertifyingPO(null); // Close the dialog immediately
  ...
  return (
    <>
      <div className="p-8 space-y-8 max-w-7xl mx-auto text-black font-bold">
       ...
       <AlertDialog open={!!certifyingPO} onOpenChange={() => setCertifyingPO(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Certify Service Completion?</AlertDialogTitle>
            <AlertDialogDescription>
              This confirms that the service "{certifyingPO?.items[0].name}" by "{certifyingPO?.supplierName}" has been rendered satisfactorily. This action is irreversible and will create a financial liability.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => certifyService(certifyingPO)} disabled={loading}>
              {loading ? <Loader2 className="animate-spin" /> : 'Yes, Certify Job'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
