...
export default function LocumPaymentsHub() {
  ...
  const pingClinicianForTin = async (locumData: LocumVoucher) => {
    setProcessingId(`ping-${locumData.staffId}`);
    try {
      const res = await fetch('/api/finance/locum/ping-tin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          hospitalId,
          clinicianId: locumData.staffId,
          clinicianName: locumData.name,
          netPayable: locumData.netPayable,
          phone: locumData.rawShifts?.[0]?.phone || '0244123456'
        })
      });
      const data = await res.json();
      if (res.ok) {
        toast({
          title: "Compliance Ping Dispatched",
          description: `SMS notification dispatched to ${locumData.name}: "GRA TIN required for ₵${locumData.netPayable.toFixed(2)} disbursement."`
        });
      } else {
        throw new Error(data.error || 'Failed to dispatch ping');
      }
    } catch (e: any) {
  ...
  const processLocumPay = async (locumData: LocumVoucher) => {
    ...
    try {
      ...
      const res = await fetch('/api/finance/locum/generate-voucher', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          hospitalId: hospitalId || 'MMH_ACCRA',
          clinicianId: locumData.staffId,
          shiftIds,
          makerUid: user?.uid,
          makerName: user?.displayName || userProfile?.name || 'Marcus Amosah Henaku'
        })
      });
      ...
      toast({
        ...
        description: `PV #${data.pvNumber} generated for ${locumData.name}. Net: GHS ${locumData.netPayable.toFixed(2)}, GRA 7.5% WHT: GHS ${locumData.whtAmount.toFixed(2)}. Routed to Checker Queue.`
      });
    } catch (e: any) {
    ...
    try {
      ...
      if (eligibleVouchers.length === 0) {
        toast({
          ...
          description: "All selected clinicians are missing GRA TINs. Update profiles or ping clinicians to proceed."
        });
        return;
      }
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to generate batch voucher');
      }

      toast({
        title: `Consolidated Batch PV Generated (${eligibleVouchers.length} Clinicians)`,
        description: `Voucher #${data.pvNumber} created with ${eligibleVouchers.length} multi-payee lines. Net: GHS ${data.netAmount?.toFixed(2)}, Total 7.5% WHT: GHS ${data.whtAmount?.toFixed(2)}. Routed to Checker.`
      });

      setSelectedVoucherIds([]);
    } catch (e: any) {
      toast({ variant: 'destructive', title: "Batch Generation Failed", description: e.message });
    } finally {
    ...
    <div className="p-6 md:p-8 bg-slate-100 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>GENERATE CONSOLIDATED BATCH PV ({selectedVoucherIds.length})</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredVouchers.map((voucher) => {
          ...
          const glInfo = voucher.role.includes('ICU') || voucher.role.includes('INTENSIVIST')
            ? { code: '5120', name: 'ICU Locum Fees' }
            : voucher.role.includes('NURSE')
            ? { code: '5125', name: 'Nursing Locum Care' }
            : { code: '5105', name: 'OPD Clinical Locum' };
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[9px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold border border-slate-200 dark:border-slate-700">
                        GL: {glInfo.code} - {glInfo.name}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {isMissingTin ? (
                  <span className="px-2.5 py-1 bg-rose-100 dark:bg-rose-950 text-rose-800 dark:text-rose-300 font-black text-[9px] uppercase rounded-lg border border-rose-300 flex items-center gap-1 animate-pulse shrink-0">
                    <AlertTriangle className="w-3 h-3 text-rose-500" /> MISSING TIN
                  </span>
                ) : (
                  <span className="px-2.5 py-1 bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 font-black text-[9px] uppercase rounded-lg border border-emerald-300 font-mono shrink-0">
                    TIN: {voucher.tin}
                  </span>
                )}
              </div>
              ...
              {isMissingTin ? (
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => pingClinicianForTin(voucher)}
                    disabled={processingId === `ping-${voucher.staffId}`}
                    className="w-1/2 py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                    title="Send automated SMS notification requesting statutory GRA TIN"
                  >
                    {processingId === `ping-${voucher.staffId}` ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                      <MessageSquare className="w-3.5 h-3.5" />
                    )}
                    <span>PING FOR TIN</span>
                  </button>
                  <button
                    type="button"
                    disabled
                    className="w-1/2 py-3 bg-slate-200 text-slate-400 dark:bg-slate-800 dark:text-slate-600 font-black text-xs uppercase tracking-wider rounded-xl cursor-not-allowed border border-slate-300 dark:border-slate-700"
                  >
                    RESTRICTED
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer disabled:opacity-50"
                >
                  ...
                  <span>GENERATE STATUTORY PAYMENT VOUCHER</span>
                </button>
              )}
            </div>
          );
